...
class MyModelModule(yd.ModelModule):
    ...
    def train_epoch_end(self):
        yd.logger.log_props(loss=self.loss.val, auc=self.metric.auc, acc=self.metric.acc, f1_score=self.metric.f1_score)
...
class MyDataModule(yd.DataModule):
    ...
    def train_loader(self):
        for train_idx in self.train_idxs:
            ...
            yield DataLoader(dataset, batch_size=BATCH_SIZE, num_workers=4, shuffle=True, drop_last=True, pin_memory=True)
